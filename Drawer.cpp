...
static const int REFRESH_COUNT = 60;	//ïΩãœÇéÊÇÈÉTÉìÉvÉãêî
...
void Drawer::flip( ) {
	...
	if ( _refresh_count == REFRESH_COUNT ){ //60ÉtÉåÅ[ÉÄñ⁄Ç»ÇÁïΩãœÇåvéZÇ∑ÇÈ
		int frame_time_sum = GetNowCount( ) - _start_time;//Ç©Ç©Ç¡ÇΩéûä‘
		double frame_time_avg = frame_time_sum / REFRESH_COUNT;//ïΩãœ
	...
	int took_time = GetNowCount( ) - _start_time;	//Ç©Ç©Ç¡ÇΩéûä‘
	int wait_time = _refresh_count * 1000 / FPS - took_time;	//ë“Ç¬Ç◊Ç´éûä‘
	if ( wait_time > 0 ) {
		Sleep( wait_time );	//ë“ã@
	}
